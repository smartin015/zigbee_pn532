...
bool Adafruit_PN532::sendCommandCheckAck(uint8_t *cmd, uint8_t cmdlen,
  ...
  // I2C TUNING — use busy-wait to avoid tickless idle stretching
  delayMicroseconds(SLOWDOWN * 1000);
...
bool Adafruit_PN532::waitready(uint16_t timeout) {
  ...
  while (!isready()) {
    ...
    // Use busy-wait delayMicroseconds — delay() is stretched by ESP-IDF tickless idle
